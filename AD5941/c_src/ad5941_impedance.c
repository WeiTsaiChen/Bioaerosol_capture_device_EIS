...
static AD5941Err AppIMPSeqCfgGen(void)
{
  ...
  if(AppIMPCfg.SweepCfg.SweepEn == bTRUE)
  {
    ...
    AD5941_SweepNext(&AppIMPCfg.SweepCfg, &AppIMPCfg.SweepNextFreq);
  ...
  HsLoopCfg.WgCfg.SinCfg.SinFreqWord = AD5941_WGFreqWordCal(sin_freq, AppIMPCfg.SysClkFreq);
  ...
  if(error == AD5941ERR_OK)
  {
    ...
    AD5941_SEQCmdWrite(AppIMPCfg.InitSeqInfo.SeqRamAddr, pSeqCmd, SeqLen);
  }
  else
    return error; /* Error */
  return AD5941ERR_OK;
...
static AD5941Err AppIMPSeqMeasureGen(void)
{
  AD5941Err error = AD5941ERR_OK;
  const uint32_t *pSeqCmd;
  uint32_t SeqLen;
  ...
  AD5941_SEQGenCtrl(bTRUE);
  AD5941_SEQGpioCtrlS(AGPIO_Pin2); /* Set GPIO1, clear others that under control */
  AD5941_SEQGenInsert(SEQ_WAIT(16*250));  /* @todo wait 250us? */
  sw_cfg.Dswitch = SWD_RCAL0;
  sw_cfg.Pswitch = SWP_RCAL0;
  sw_cfg.Nswitch = SWN_RCAL1;
  sw_cfg.Tswitch = SWT_RCAL1|SWT_TRTIA;
  AD5941_SWMatrixCfgS(&sw_cfg);
	AD5941_AFECtrlS(AFECTRL_HSTIAPWR|AFECTRL_INAMPPWR|AFECTRL_EXTBUFPWR|\
                AFECTRL_WG|AFECTRL_DACREFPWR|AFECTRL_HSDACPWR|\
                AFECTRL_SINC2NOTCH, bTRUE);
  AD5941_AFECtrlS(AFECTRL_WG|AFECTRL_ADCPWR, bTRUE);  /* Enable Waveform generator */
  //delay for signal settling DFT_WAIT
  AD5941_SEQGenInsert(SEQ_WAIT(16*10));
  AD5941_AFECtrlS(AFECTRL_ADCCNV|AFECTRL_DFT, bTRUE);  /* Start ADC convert and DFT */
  AD5941_SEQGenInsert(SEQ_WAIT(WaitClks));
  //wait for first data ready
  AD5941_AFECtrlS(AFECTRL_ADCPWR|AFECTRL_ADCCNV|AFECTRL_DFT|AFECTRL_WG, bFALSE);  /* Stop ADC convert and DFT */
  ...
  AD5941_SWMatrixCfgS(&sw_cfg);
  AD5941_AFECtrlS(AFECTRL_ADCPWR|AFECTRL_WG, bTRUE);  /* Enable Waveform generator */
  AD5941_SEQGenInsert(SEQ_WAIT(16*10));  //delay for signal settling DFT_WAIT
  AD5941_AFECtrlS(AFECTRL_ADCCNV|AFECTRL_DFT, bTRUE);  /* Start ADC convert and DFT */
  ...
  AD5941_SEQGpioCtrlS(0); /* Clr GPIO1 */
  ...
  if(error == AD5941ERR_OK)
  {
    ...
    /* Write command to SRAM */
    AD5941_SEQCmdWrite(AppIMPCfg.MeasureSeqInfo.SeqRamAddr, pSeqCmd, SeqLen);
  }
  else
    return error; /* Error */
  return AD5941ERR_OK;
...
int32_t AppIMPInit(uint32_t *pBuffer, uint32_t BufferSize)
{
  ...
  /* Reconfigure FIFO */
  AD5941_FIFOCtrlS(FIFOSRC_DFT, bFALSE);									/* Disable FIFO firstly */
  ...
  AD5941_SEQInfoCfg(&AppIMPCfg.InitSeqInfo);
  seq_cfg.SeqEnable = bTRUE;
  AD5941_SEQCfg(&seq_cfg);  /* Enable sequencer */
  AD5941_SEQMmrTrig(AppIMPCfg.InitSeqInfo.SeqId);
  while(AD5941_INTCTestFlag(AFEINTC_1, AFEINTSRC_ENDSEQ) == bFALSE);
  
  /* Measurement sequence  */
  AppIMPCfg.MeasureSeqInfo.WriteSRAM = bFALSE;
  AD5941_SEQInfoCfg(&AppIMPCfg.MeasureSeqInfo);

  seq_cfg.SeqEnable = bTRUE;
  AD5941_SEQCfg(&seq_cfg);  /* Enable sequencer, and wait for trigger */
  AD5941_ClrMCUIntFlag();   /* Clear interrupt flag generated before */

  AD5941_AFEPwrBW(AppIMPCfg.PwrMod, AFEBW_250KHZ);
...
int32_t AppIMPRegModify(int32_t * const pData, uint32_t *pDataCount)
{
  ...
  {
    AD5941_WGFreqCtrlS(AppIMPCfg.SweepNextFreq, AppIMPCfg.SysClkFreq);
  }
  return AD5941ERR_OK;
}
...
int32_t AppIMPISR(void *pBuff, uint32_t *pCount)
{
  ...
  *pCount = 0;
  ...
  if(AD5941_WakeUp(10) > 10)  /* Wakeup AFE by read register, read 10 times at most */
    return AD5941ERR_WAKEUP;  /* Wakeup Failed */
  AD5941_SleepKeyCtrlS(SLPKEY_LOCK);  /* Prohibit AFE to enter sleep mode. */

  if(AD5941_INTCTestFlag(AFEINTC_0, AFEINTSRC_DATAFIFOTHRESH) == bTRUE)
  {
    /* Now there should be 4 data in FIFO */
    FifoCnt = (AD5941_FIFOGetCnt()/4)*4;
