 ...
 * @return none.
*/
AD5941Err AppRAMPGetCfg(void *pCfg)
    {
    if(pCfg)
        {
 ...
 * @return none.
*/
AD5941Err AppRAMPCtrl(uint32_t Command, void *pPara)
    {
    switch (Command)
        {
        case APPCTRL_START:
            ...
            if(AD5941_WakeUp(10) > 10)  /* Wakeup AFE by read register, read 10 times at most */
                return AD5941ERR_WAKEUP;  /* Wakeup Failed */
            if(AppRAMPCfg.RAMPInited == bFALSE)
                return AD5941ERR_APPERROR;
            /**
...
*/
static AD5941Err AppRAMPSeqInitGen(void)
    {
    ...
    /* Sequence end. */
    AD5941_SEQGenInsert(SEQ_STOP()); /* Add one extra command to disable sequencer for initialization sequence because we only want it to run one time. */

    /* Stop sequence generator here */
    AD5941_SEQGenCtrl(bFALSE); /* Stop sequencer generator */
    error = AD5941_SEQGenFetchSeq(&pSeqCmd, &SeqLen);
    if(error == AD5941ERR_OK)
        {
        AD5941_StructInit(&AppRAMPCfg.InitSeqInfo, sizeof(AppRAMPCfg.InitSeqInfo));
        if(SeqLen >= AppRAMPCfg.MaxSeqLen)
            return AD5941ERR_SEQLEN;
        ...
        AD5941_SEQInfoCfg(&AppRAMPCfg.InitSeqInfo);
        }
    else
        return error; /* Error */
    return AD5941ERR_OK;
 ...
 * @return return error code.
*/
static AD5941Err AppRAMPSeqADCCtrlGen(void)
    {
    AD5941Err error = AD5941ERR_OK;
    const uint32_t *pSeqCmd;
    uint32_t SeqLen;
    ...
    AD5941_ClksCalculate(&clks_cal, &WaitClks);

    AD5941_SEQGenCtrl(bTRUE);
    AD5941_SEQGpioCtrlS(AGPIO_Pin2);
    AD5941_AFECtrlS(AFECTRL_ADCPWR, bTRUE);
    AD5941_SEQGenInsert(SEQ_WAIT(16 * 250)); /* wait 250us for reference power up */
    AD5941_AFECtrlS(AFECTRL_ADCCNV, bTRUE);  /* Start ADC convert and DFT */
    AD5941_SEQGenInsert(SEQ_WAIT(WaitClks));  /* wait for first data ready */
    AD5941_AFECtrlS(AFECTRL_ADCPWR | AFECTRL_ADCCNV, bFALSE); /* Stop ADC */
    AD5941_SEQGpioCtrlS(0);
    AD5941_EnterSleepS();/* Goto hibernate */
    /* Sequence end. */
    error = AD5941_SEQGenFetchSeq(&pSeqCmd, &SeqLen);
    AD5941_SEQGenCtrl(bFALSE); /* Stop sequencer generator */
    ...
    if(error == AD5941ERR_OK)
        {
        AD5941_StructInit(&AppRAMPCfg.ADCSeqInfo, sizeof(AppRAMPCfg.ADCSeqInfo));
        ...
        AD5941_SEQInfoCfg(&AppRAMPCfg.ADCSeqInfo);
        }
    else
        return error; /* Error */
    return AD5941ERR_OK;
 ...
 * @return return error code.
*/
static AD5941Err RampDacRegUpdate(uint32_t *pDACData) // NOTE
    {
 ...
 *
 * */
static AD5941Err AppRAMPSeqDACCtrlGen(void)
    {
...
#define CURRBLK_BLK1      1   /* Current block is BLOCK1 */
    ...
    if(AppRAMPCfg.bFirstDACSeq == bTRUE)
        ...
        if(DACSeqLenMax < SEQLEN_ONESTEP * 4)
            return AD5941ERR_SEQLEN;  /* No enough sequencer SRAM available */
    ...
    if(StepsRemainning == 0) return AD5941ERR_OK; /* Done. */
    ...
    for(i = 0; i < StepsThisBlock - 1; i++)
        ...
        AD5941_SEQCmdWrite(CurrAddr, SeqCmdBuff, SEQLEN_ONESTEP);
    ...
    if(AppRAMPCfg.bFirstDACSeq)
        ...
        AD5941_SEQInfoCfg(&AppRAMPCfg.DACSeqInfo);
        }
    return AD5941ERR_OK;
    }
 ...
 * @return return error code.
*/
static AD5941Err AppRAMPRtiaCal(void)
    {
    fImpPol_Type RtiaCalValue;  /* Calibration result */ // Caution!
    LPRTIACal_Type lprtia_cal;
    AD5941_StructInit(&lprtia_cal, sizeof(lprtia_cal));
    ...
    AD5941_LPRtiaCal(&lprtia_cal, &RtiaCalValue);
 ...
 * @return return error code.
*/
AD5941Err AppRAMPInit(uint32_t *pBuffer, uint32_t BufferSize)
    {
    AD5941Err error = AD5941ERR_OK;
    ...
    if((AppRAMPCfg.RAMPInited == bFALSE) || \
        ...
        {
        if(pBuffer == 0)  return AD5941ERR_PARA;
        if(BufferSize == 0) return AD5941ERR_PARA;
        ...
        AD5941_SEQGenInit(pBuffer, BufferSize);
        ...
        if(error != AD5941ERR_OK) return error;
    ...
    AD5941_FIFOCtrlS(FIFOSRC_SINC3, bFALSE);        /* Disable FIFO firstly */
    ...
    AD5941_SEQCtrlS(bTRUE); /* Enable sequencer */
    AD5941_SEQMmrTrig(AppRAMPCfg.InitSeqInfo.SeqId);
    ...
    AD5941_SEQCtrlS(bFALSE);
    AD5941_WriteReg(REG_AFE_SEQCNT, 0);
    AD5941_SEQCtrlS(bTRUE);   /* Enable sequencer, and wait for trigger */
    AD5941_ClrMCUIntFlag();   /* Clear interrupt flag generated before */

    AD5941_AFEPwrBW(AFEPWR_LP, AFEBW_250KHZ); /* Set to low power mode */
...
static int32_t AppRAMPDataProcess(int32_t *const pData, uint32_t *pDataCount)
    ...
    for(i = 0; i < datacount; i++)
        {
        pData[i] &= 0xffff;	
				temp = -AD5941_ADCCode2Volt(pData[i], AppRAMPCfg.AdcPgaGain, AppRAMPCfg.ADCRefVolt); // Convert the code into voltage from here.	
 ...
 * @return return error code.
*/
AD5941Err AppRAMPISR(void *pBuff, uint32_t *pCount)
    {
    uint32_t BuffCount;
    uint32_t FifoCnt;
    BuffCount = *pCount;
    ...
    if(AD5941_WakeUp(10) > 10)  /* Wakeup AFE by read register, read 10 times at most */
        return AD5941ERR_WAKEUP;  /* Wakeup Failed */
    AD5941_SleepKeyCtrlS(SLPKEY_LOCK);
    *pCount = 0;
    IntFlag = AD5941_INTCGetFlag(AFEINTC_0);
    if(IntFlag & AFEINTSRC_CUSTOMINT0)          /* High priority. */
        {
        AD5941Err error;
        AD5941_INTCClrFlag(AFEINTSRC_CUSTOMINT0);
        error = AppRAMPSeqDACCtrlGen();
        if(error != AD5941ERR_OK) return error;
        AD5941_SleepKeyCtrlS(SLPKEY_UNLOCK);
        //AD5941_EnterSleepS(); /* If there is need to do AFE re-configure, do it here when AFE is in active state */
        }
    if(IntFlag & AFEINTSRC_DATAFIFOTHRESH)
        {
        FifoCnt = AD5941_FIFOGetCnt();

        if(FifoCnt > BuffCount)
            {
            ///@todo buffer is limited.
            }
        AD5941_FIFORd((uint32_t *)pBuff, FifoCnt);
        AD5941_INTCClrFlag(AFEINTSRC_DATAFIFOTHRESH);
