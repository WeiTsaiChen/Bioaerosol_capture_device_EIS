...
#include "ad5941_idef.h"
#include "..\ad5941_udef.h"

SPIParams_Type SPI_Para;

volatile static UINT32 ucInterrupted = 0;       /* Flag to indicate interrupt occurred */
...
#ifndef CHIPSEL_M355
static uint32_t AD5941_SPIReadReg(uint16_t RegAddr);
static void AD5941_SPIWriteReg(uint16_t RegAddr, uint32_t RegData);
#else
static uint32_t AD5941_D2DReadReg(uint16_t RegAddr);
static void AD5941_D2DWriteReg(uint16_t RegAddr, uint32_t RegData);
#endif

/** 
 * @addtogroup AD5941_Library
 *  The library functions, structures and constants.
 * @{
 *    @defgroup AD5941_Functions
 *    @{
...
struct
{
  ...
  AD5941Err LastError;          /**< The last error message. */
 ...
 * @return None;
*/
void AD5941_SEQGenInsert(uint32_t CmdWord)
{
  uint32_t temp;
  ...
  if(temp < SeqGenDB.BufferSize)
  {
    ...
    SeqGenDB.LastError = AD5941ERR_BUFF;
}
 ...
 * @return Return AD5941ERR_OK if register found in data-base. Otherwise return AD5941ERR_SEQREG.
*/
static AD5941Err AD5941_SEQGenSearchReg(uint32_t RegAddr, uint32_t *pIndex)
{
  uint32_t i;
  ...
  return AD5941ERR_SEQREG;
}

/**
 * @brief Get the register default value by SPI read. This function requires AD5941 is in active state, otherwise we cannot get the default register value.
 * @param RegAddr: The register address.
 * @param pRegData: Pointer to a variable to store register default value.
 * @return Return AD5941ERR_OK.
*/
static AD5941Err AD5941_SEQGenGetRegDefault(uint32_t RegAddr, uint32_t *pRegData)
{
#ifdef CHIPSEL_M355
  *pRegData = AD5941_D2DReadReg(RegAddr);
#else
  *pRegData = AD5941_SPIReadReg(RegAddr);
#endif
  return AD5941ERR_OK;
}
 ...
 * @return Return None.
*/
static void AD5941_SEQRegInfoInsert(uint16_t RegAddr, uint32_t RegData)
{
  uint32_t temp;
  temp = SeqGenDB.RegCount + SeqGenDB.SeqLen;
...
*/
static uint32_t AD5941_SEQReadReg(uint16_t RegAddr)
{
  uint32_t RegIndex, RegData;
  
  if(AD5941_SEQGenSearchReg(RegAddr, &RegIndex) != AD5941ERR_OK)
  {
    /* There is no record in data-base, read the default value. */
    AD5941_SEQGenGetRegDefault(RegAddr, &RegData);
    AD5941_SEQRegInfoInsert(RegAddr, RegData);
  }
  else
  {
 ...
 * @return Return None.
*/
static void AD5941_SEQWriteReg(uint16_t RegAddr, uint32_t RegData)
{
  uint32_t RegIndex;
  
  if(RegAddr > 0x21ff)
  {
    SeqGenDB.LastError = AD5941ERR_ADDROR;  /* address out of range  */
    return;
  }
  ...
  if(AD5941_SEQGenSearchReg(RegAddr, &RegIndex) == AD5941ERR_OK)
  {
    ...
    /* Generate Sequence command */
    AD5941_SEQGenInsert(SEQ_WR(RegAddr, RegData));
  }
  else
  {
    AD5941_SEQRegInfoInsert(RegAddr, RegData);
 ...
 * @return Return None.
*/
void AD5941_SEQGenInit(uint32_t *pBuffer, uint32_t BufferSize)
{
  ...
  SeqGenDB.RegCount = 0;
  SeqGenDB.LastError = AD5941ERR_OK;
...
*/
AD5941Err AD5941_SEQGenFetchSeq(const uint32_t **ppSeqCmd, uint32_t *pSeqLen)
{
  AD5941Err lasterror;
  ...
  //SeqGenDB.LastError = AD5941ERR_OK;  /* Clear error message */
  return lasterror;
}

/**
 * @brief Start or stop the sequencer generator. Once started, the register write will be recorded to sequencer generator.
 *        Once it's disabled, the register write is written to AD5941 directly by SPI bus.
 * @param bFlag: Enable or disable sequencer generator.
 * @return Return None.
*/
void AD5941_SEQGenCtrl(BoolFlag bFlag)
{
 ...
 *       they may need less clocks to generate required number of output. Use function @ref AD5941_AFECtrlS to control these blocks.
  ...
  *pClocks = 0;
  ...
  switch(pFilterInfo->DataType)
  {
    ...
    case DATATYPE_SINC2: 
      ...
      AD5941_ClksCalculate(pFilterInfo, &temp);
    ...
    case DATATYPE_NOTCH:
    {
      ...
      if(AD5941_Notch50HzAvailable(&filter, &dl_50)){
        dl += dl_50 - 1;
      }
      if(AD5941_Notch60HzAvailable(&filter, &dl_60)){
        dl += dl_60 - 1;
      }
      pFilterInfo->DataType = DATATYPE_SINC2;
...
/**
   @brief void AD5941_SweepNext(SoftSweepCfg_Type *pSweepCfg, float *pNextFreq)
...
**/
float AD5941_ADCCode2Volt(uint32_t code, uint32_t ADCPga, float VRef1p82)
{
...
/**
 * @defgroup SPI_Block
 * @brief Functions to communicate with AD5941 registers following AD5941 SPI protocols
 * @{
 * 
 * @defgroup SPI_Block_Functions
 * @brief The basic SPI protocols. All functions are basic on AD5941_ReadWriteNBytes which
 *        provided by user.
 *        
 *  ##SPI basic protocol
...
**/
static unsigned char AD5941_ReadWrite8B(unsigned char data)
{
   uint8_t tx[1], rx[1];
   tx[0] = data;
   AD5941_ReadWriteNBytes(tx,rx,1);
   return rx[0];
}
...
/**
  ...
  @return received data.
**/
static uint16_t AD5941_ReadWrite16B(uint16_t data)
{
   uint8_t SendBuffer[2];
   uint8_t RecvBuffer[2];
   SendBuffer[0] = data>>8;
   SendBuffer[1] = data&0xff;
   AD5941_ReadWriteNBytes(SendBuffer,RecvBuffer,2);
 ...
 * @return received data.
**/
static uint32_t AD5941_ReadWrite32B(uint32_t data)
{
   ...
   AD5941_ReadWriteNBytes(SendBuffer,RecvBuffer,4);
...
{  
  ...
  AD5941_CsClr();
  AD5941_ReadWrite8B(SPICMD_SETADDR);
  AD5941_Delay10us(10);
  
//  AD5941_ReadWrite16B(RegAddr);
  
  AD5941_ReadWrite8B(RegAddr>>8);
  AD5941_Delay10us(10);
  AD5941_ReadWrite8B(RegAddr&0xFF);
  
  AD5941_Delay10us(100);
  
  AD5941_CsSet();
  /* Add delay here to meet the SPI timing. */
  AD5941_CsClr();
  AD5941_ReadWrite8B(SPICMD_WRITEREG);
  if(((RegAddr>=0x1000)&&(RegAddr<=0x3014)))
  {
    /* Code original, if the transferring rate can not reach the limit, commend */
    // AD5941_ReadWrite32B(RegData);
    
    /* If required, use the code below */
    AD5941_ReadWrite8B(RegData>>24);
    AD5941_Delay10us(10);
    AD5941_ReadWrite8B(RegData>>16&0xFF);
    AD5941_Delay10us(10);
    AD5941_ReadWrite8B(RegData>>8&0xFF);
    AD5941_Delay10us(10);
    AD5941_ReadWrite8B(RegData&0xFF);
    
  }
  else
  {
    /* Code original, if the transferring rate can not reach the limit, commend */
    //    AD5941_ReadWrite16B(RegData);
    
    /* If required, use the code below */
    AD5941_ReadWrite8B(RegData>>8);
    AD5941_Delay10us(10);
    AD5941_ReadWrite8B(RegData&0xFF);
  }
    
  AD5941_Delay10us(150);
...
//  AD5941_ReadWrite16B(RegAddr);
  ...
  /* Read it */
  AD5941_CsClr();
  AD5941_ReadWrite8B(SPICMD_READREG);
  AD5941_Delay10us(10);
  AD5941_ReadWrite8B(0);  //Dummy read
  ...
  {
      /* Code original, if the transferring rate can not reach the limit, commend */
      // Data = AD5941_ReadWrite32B(0);

      /* If required, use the code below */
      /* Dummy read, don't use */
      temp0 = AD5941_ReadWrite8B(0);
      SPI_Para.SPI_RW_DATA_8_List[0] = temp0;
      AD5941_Delay10us(10);
      
      /*Start reading the data */
      temp1 = AD5941_ReadWrite8B(0);
      SPI_Para.SPI_RW_DATA_8_List[1] = temp1;
      AD5941_Delay10us(10);
      temp2 = AD5941_ReadWrite8B(0);
      SPI_Para.SPI_RW_DATA_8_List[2] = temp2;
      AD5941_Delay10us(10);
      temp3 = AD5941_ReadWrite8B(0);
      SPI_Para.SPI_RW_DATA_8_List[3] = temp3;
      AD5941_Delay10us(10);
      temp4 = AD5941_ReadWrite8B(0);
      SPI_Para.SPI_RW_DATA_8_List[4] = temp4;
      AD5941_Delay10us(10);
      
      Data = temp1<<24 | temp2<<16 | temp3<<8 | temp4;
      SPI_Para.SPI_READ_DATA = Data;
  }
  else
  {
      /* Code original, if the transferring rate can not reach the limit, commend */
//    Data = AD5941_ReadWrite16B(0);
...
void AD5941_FIFORd(uint32_t *pBuffer, uint32_t uiReadCount)   
{
  /* Use function AD5941_SPIReadReg to read REG_AFE_DATAFIFORD is also one method. */
   uint32_t i;
   ...
   if(uiReadCount < 3)
   {
      ...
      // AD5941_ReadWrite16B(REG_AFE_DATAFIFORD);
      
      AD5941_ReadWrite8B(REG_AFE_DATAFIFORD>>8);
      AD5941_Delay10us(10);
      AD5941_ReadWrite8B(REG_AFE_DATAFIFORD&0xFF);
      AD5941_Delay10us(10);
      ...
      for(i=0;i<uiReadCount;i++)
      {
         ...
         AD5941_ReadWrite8B(SPICMD_READREG);
         AD5941_Delay10us(10);
         AD5941_ReadWrite8B(0);//Write Host status/Don't care
         AD5941_Delay10us(10);
         
         // pBuffer[i] = AD5941_ReadWrite32B(0);
         
         /* Combination of FIFO data */          
         for(int j=0; j<5; i++)
         {
          temp_list[j] = AD5941_ReadWrite8B(0);  
          AD5941_Delay10us(10);
         }
         
         Buffer_value = temp_list[1]<<24 | temp_list[2]<<16 | temp_list[3]<<8 | temp_list[4];
         pBuffer[i] = Buffer_value;
   ...
   {
      AD5941_CsClr();
      AD5941_ReadWrite8B(SPICMD_READFIFO);
      AD5941_Delay10us(10);
      
      /* 6 dummy write before valid data read back */
      for(i=0;i<6;i++)
      {
        AD5941_ReadWrite8B(0);
        AD5941_Delay10us(10);
      }
      ...
      for(i=0;i<uiReadCount-2;i++)
      {
        ...
        // pBuffer[i] = AD5941_ReadWrite32B(0); /*Offset is 0, so we always read DATAFIFORD register */
        
        /* Combination of FIFO data */
        for(int k=0; k<5; k++)
        {
          temp_list[k] = AD5941_ReadWrite8B(0); 
          AD5941_Delay10us(10);
        }
      ...
      // pBuffer[i++] = AD5941_ReadWrite32B(0x44444444);
      // pBuffer[i] = AD5941_ReadWrite32B(0x44444444);
      
      /* Read back last two FIFO data with none-zero offset - first read */
      for(int l=0; l<5; l++)
      {
        temp_list[l] = AD5941_ReadWrite8B(0x44);
        AD5941_Delay10us(10);
      }
      ...
      /* Read back last two FIFO data with none-zero offset - second read */
      for(int m=0; m<5; m++)
      {
        temp_list[m] = AD5941_ReadWrite8B(0x44);
        AD5941_Delay10us(10);
      }
 ...
 *        Otherwise, the data is written to AD5941 by SPI.
 * @param RegAddr: The register address.
 * @param RegData: The register data.
 * @return Return None.
**/
...
#endif
#ifdef CHIPSEL_M355
    AD5941_D2DWriteReg(RegAddr, RegData);
#else
    AD5941_SPIWriteReg(RegAddr, RegData);
#endif
}
...
/**
 * @brief Initialize AD5941. This function must be called whenever there is reset(Software Reset or Hardware reset or Power up) happened.
 *        This function is used to put AD5941 to correct state.
 * @return return None
**/
void AD5941_Initialize(void)
{
  ...
  {
    {0x0908, 0x02c9}, // reg_addr, reg_data
    {0x0c08, 0x206C},
    {0x21F0, 0x0010},
#ifndef CHIPSEL_M355
    /* This is AD5941 */
  ...
  SeqGenDB.EngineStart = bFALSE;
#ifndef CHIPSEL_M355
  AD5941_CsSet(); /* Pull high CS in case it's low */
#endif
  for(i=0; i<sizeof(RegTable)/sizeof(RegTable[0]); i++)
    AD5941_WriteReg(RegTable[i].reg_addr, RegTable[i].reg_data);
  
  AD5941_Delay10us(10);
  
  i = AD5941_ReadReg(REG_AFECON_CHIPID);      
  
  SPI_Para.SPI_READ_DATA = i;
  // AD5941_Delay10us(1*1000);
  ...
  {
    printf("CHIPID read error:0x%04x. AD5941 is not present?\n", i);
    SPI_Para.SPI_READ_DATA = 0xFF;
    while(1);
  }
#ifdef CHIPSEL_M355
  ...
  ADI_Print("AD5941LIB Version:v%d.%d.%d\n", AD5941LIB_VER_MAJOR, AD5941LIB_VER_MINOR, AD5941LIB_VER_PATCH);
#endif
}
...
void AD5941_AFECtrlS(uint32_t AfeCtrlSet, BoolFlag State)
{
  /* Check parameters */
  uint32_t tempreg;
  tempreg = AD5941_ReadReg(REG_AFE_AFECON);
  if (State == bTRUE) {
  ...
  AD5941_WriteReg(REG_AFE_AFECON, tempreg);
}
/** When LP mode is enabled, some functions are under control of LPMODECON, rather than original registers.  */
/** @warning LPMODE is key protected, this function only takes effect after AD5941_LPModeEnS(bTRUE) */
/**
...
AD5941Err AD5941_LPModeCtrlS(uint32_t EnSet)
{
  ...
  tempreg = AD5941_ReadReg(REG_AFE_LPMODECON);
  /* Enable selected set */
  {
  ...
  AD5941_WriteReg(REG_AFE_LPMODECON, tempreg);
...
/**
   ...
   @return return none.
*/
void AD5941_AFEPwrBW(uint32_t AfePwr, uint32_t AfeBw)
{
  //check parameters
  uint32_t tempreg;
  tempreg = AfePwr;
  tempreg |= AfeBw << BITP_AFE_PMBW_SYSBW;
  AD5941_WriteReg(REG_AFE_PMBW, tempreg);
}
...
void AD5941_REFCfgS(AFERefCfg_Type *pBufCfg)
{
  ...
  tempreg = AD5941_ReadReg(REG_AFE_BUFSENCON);
  ...
  AD5941_WriteReg(REG_AFE_BUFSENCON, tempreg);
  ...
  AD5941_WriteReg(REG_AFE_LPREFBUFCON, tempreg);
}
/**
...
{
  AD5941_HSDacCfgS(&pHsLoopCfg->HsDacCfg);
  AD5941_HSTIACfgS(&pHsLoopCfg->HsTiaCfg);
  AD5941_SWMatrixCfgS(&pHsLoopCfg->SWMatCfg);
  AD5941_WGCfgS(&pHsLoopCfg->WgCfg);
}
...
{
  AD5941_WriteReg(REG_AFE_DSWFULLCON, pSwMatrix->Dswitch);
  AD5941_WriteReg(REG_AFE_PSWFULLCON, pSwMatrix->Pswitch);
  AD5941_WriteReg(REG_AFE_NSWFULLCON, pSwMatrix->Nswitch);
  AD5941_WriteReg(REG_AFE_TSWFULLCON, pSwMatrix->Tswitch);
  AD5941_WriteReg(REG_AFE_SWCON, BITM_AFE_SWCON_SWSOURCESEL); /* Update switch configuration */
}
...
void AD5941_HSDacCfgS(HSDACCfg_Type *pHsDacCfg)
{
  ...
  AD5941_WriteReg(REG_AFE_HSDACCON, tempreg);
}


static void __AD5941_SetDExRTIA(uint32_t DExPin, uint32_t DeRtia, uint32_t DeRload)
{
  uint32_t tempreg;
...
//Rload  0      10    30    50    100 
  ...
  tempreg |= DeRload;
  if(DExPin) //DE1
    AD5941_WriteReg(REG_AFE_DE1RESCON, tempreg);
  else  //DE0
    AD5941_WriteReg(REG_AFE_DE0RESCON, tempreg);
}
...
AD5941Err AD5941_HSTIACfgS(HSTIACfg_Type *pHsTiaCfg)
{
  uint32_t tempreg;
  //Check parameters
  if(pHsTiaCfg == NULL) return AD5941ERR_NULLP;
    /* Available parameter is 1k, 5k,...,160k, short, OPEN */
  if(pHsTiaCfg->HstiaDeRtia < HSTIADERTIA_1K)
    return AD5941ERR_PARA;
  if(pHsTiaCfg->HstiaDeRtia > HSTIADERTIA_OPEN)
    return AD5941ERR_PARA;  /* Parameter is invalid */

  if(pHsTiaCfg->HstiaDeRload > HSTIADERLOAD_OPEN)
    return AD5941ERR_PARA;  /* Available parameter is OPEN, 0R,..., 100R */

  tempreg = 0;
  tempreg |= pHsTiaCfg->HstiaBias;
  AD5941_WriteReg(REG_AFE_HSTIACON, tempreg);
  /* HSRTIACON */
  ...
  AD5941_WriteReg(REG_AFE_HSRTIACON, tempreg);
  /* DExRESCON */
  __AD5941_SetDExRTIA(0, pHsTiaCfg->HstiaDeRtia, pHsTiaCfg->HstiaDeRload);
#ifdef CHIPSEL_M355
  __AD5941_SetDExRTIA(1, pHsTiaCfg->HstiaDe1Rtia, pHsTiaCfg->HstiaDe1Rload);
#endif
...
{
  uint32_t tempreg;
  tempreg = AD5941_ReadReg(REG_AFE_HSRTIACON);
...
void AD5941_WGCfgS(WGCfg_Type *pWGInit)
{
  ...
  if(pWGInit->WgType == WGTYPE_SIN)
  {
    ...
    AD5941_WriteReg(REG_AFE_WGFCW, pWGInit->SinCfg.SinFreqWord);
    AD5941_WriteReg(REG_AFE_WGAMPLITUDE, pWGInit->SinCfg.SinAmplitudeWord);
    AD5941_WriteReg(REG_AFE_WGOFFSET, pWGInit->SinCfg.SinOffsetWord);
    AD5941_WriteReg(REG_AFE_WGPHASE, pWGInit->SinCfg.SinPhaseWord);
  }
  else if(pWGInit->WgType == WGTYPE_TRAPZ)
  {
    /* Configure Trapezoid Generator */
    AD5941_WriteReg(REG_AFE_WGDCLEVEL1, pWGInit->TrapzCfg.WGTrapzDCLevel1);
    AD5941_WriteReg(REG_AFE_WGDCLEVEL2, pWGInit->TrapzCfg.WGTrapzDCLevel2);
    AD5941_WriteReg(REG_AFE_WGDELAY1, pWGInit->TrapzCfg.WGTrapzDelay1);
    AD5941_WriteReg(REG_AFE_WGDELAY2, pWGInit->TrapzCfg.WGTrapzDelay2);
    AD5941_WriteReg(REG_AFE_WGSLOPE1, pWGInit->TrapzCfg.WGTrapzSlope1);
    AD5941_WriteReg(REG_AFE_WGSLOPE2, pWGInit->TrapzCfg.WGTrapzSlope2);
  }
  else
  {
    /* Write DAC data. It's only have effect when WgType set to WGTYPE_MMR */ 
    AD5941_WriteReg(REG_AFE_HSDACDAT, pWGInit->WgCode);
  }
  tempreg = 0;
  ...
  AD5941_WriteReg(REG_AFE_WGCON, tempreg);
}
 ...
 * @return return none.
*/
AD5941Err AD5941_WGDACCodeS(uint32_t code)
{
  code &= 0xfff;
  AD5941_WriteReg(REG_AFE_HSDACDAT, code);
  return AD5941ERR_OK;
}
...
{
  uint32_t freq_word;
  freq_word = AD5941_WGFreqWordCal(SinFreqHz, WGClock);
  AD5941_WriteReg(REG_AFE_WGFCW, freq_word);
}
...
{
  AD5941_LPDACCfgS(&pLpLoopCfg->LpDacCfg);
  AD5941_LPAMPCfgS(&pLpLoopCfg->LpAmpCfg);
}
...
void AD5941_LPDACCfgS(LPDACCfg_Type *pLpDacCfg)
{
  ...
  {
    AD5941_WriteReg(REG_AFE_LPDACCON0, tempreg);
    AD5941_LPDAC0WriteS(pLpDacCfg->DacData12Bit, pLpDacCfg->DacData6Bit);
    AD5941_WriteReg(REG_AFE_LPDACSW0, pLpDacCfg->LpDacSW|BITM_AFE_LPDACSW0_LPMODEDIS);  /* Overwrite LPDACSW settings. On Si1, this register is not accessible. */
  }
  else
  {
    AD5941_WriteReg(REG_AFE_LPDACCON1, tempreg);
    AD5941_LPDAC1WriteS(pLpDacCfg->DacData12Bit, pLpDacCfg->DacData6Bit);
    AD5941_WriteReg(REG_AFE_LPDACSW1, pLpDacCfg->LpDacSW|BITM_AFE_LPDACSW0_LPMODEDIS);  /* Overwrite LPDACSW settings. On Si1, this register is not accessible. */
  }
}
...
void AD5941_LPDACWriteS(uint16_t Data12Bit, uint8_t Data6Bit)
{
  ...
  Data6Bit &= 0x3f;
  Data12Bit &= 0xfff;
  AD5941_WriteReg(REG_AFE_LPDACDAT0, ((uint32_t)Data6Bit<<12)|Data12Bit);
}
...
void AD5941_LPDAC1WriteS(uint16_t Data12Bit, uint8_t Data6Bit)
{
  ...
  Data6Bit &= 0x3f;
  Data12Bit &= 0xfff;
  AD5941_WriteReg(REG_AFE_LPDACDAT1, ((uint32_t)Data6Bit<<12)|Data12Bit);
}
...
void AD5941_LPAMPCfgS(LPAmpCfg_Type *pLpAmpCfg)
{
  ...
  {
    AD5941_WriteReg(REG_AFE_LPTIACON0, tempreg);
    AD5941_WriteReg(REG_AFE_LPTIASW0, pLpAmpCfg->LpTiaSW);
  }
  else
  {
    AD5941_WriteReg(REG_AFE_LPTIACON1, tempreg);
    AD5941_WriteReg(REG_AFE_LPTIASW1, pLpAmpCfg->LpTiaSW);
  }
}
/**
...
{
  AD5941_ADCBaseCfgS(&pDSPCfg->ADCBaseCfg);
  AD5941_ADCFilterCfgS(&pDSPCfg->ADCFilterCfg);
  AD5941_ADCDigCompCfgS(&pDSPCfg->ADCDigCompCfg);
  AD5941_DFTCfgS(&pDSPCfg->DftCfg);
  AD5941_StatisticCfgS(&pDSPCfg->StatCfg);
}

/**
   @brief Read AD5941 generated data like ADC and DFT etc.
...
uint32_t AD5941_ReadAfeResult(uint32_t AfeResultSel)
{
  ...
  switch (AfeResultSel)
  {
    case AFERESULT_SINC3:
      rd = AD5941_ReadReg(REG_AFE_ADCDAT);
      break;
    case AFERESULT_SINC2:
      rd = AD5941_ReadReg(REG_AFE_SINC2DAT);
      break;
    case AFERESULT_TEMPSENSOR:
      rd = AD5941_ReadReg(REG_AFE_TEMPSENSDAT);
      break;
    case AFERESULT_DFTREAL:
      rd = AD5941_ReadReg(REG_AFE_DFTREAL);
      break;
    case AFERESULT_DFTIMAGE:
      rd = AD5941_ReadReg(REG_AFE_DFTIMAG);
      break;
    case AFERESULT_STATSMEAN:
      rd = AD5941_ReadReg(REG_AFE_STATSMEAN);
      break;
    case AFERESULT_STATSVAR:
      rd = AD5941_ReadReg(REG_AFE_STATSVAR);
      break;
  }
...
void AD5941_ADCBaseCfgS(ADCBaseCfg_Type *pADCInit)
{
  ...
  AD5941_WriteReg(REG_AFE_ADCCON, tempreg);
}
...
void AD5941_ADCFilterCfgS(ADCFilterCfg_Type *pFiltCfg)
{
  ...
  tempreg = AD5941_ReadReg(REG_AFE_ADCFILTERCON);
  ...
  /**
   * Average filter is enabled when DFT source is @ref DFTSRC_AVG in function @ref AD5941_DFTCfgS.
...
void AD5941_ADCMuxCfgS(uint32_t ADCMuxP, uint32_t ADCMuxN)
{
  ...
  tempreg = AD5941_ReadReg(REG_AFE_ADCCON);
...
{
  //PARA_CHECK((AfeResultSel));
  AD5941_WriteReg(REG_AFE_ADCMIN, pCompCfg->ADCMin);
  AD5941_WriteReg(REG_AFE_ADCMINSM, pCompCfg->ADCMinHys);
  AD5941_WriteReg(REG_AFE_ADCMAX, pCompCfg->ADCMax);
  AD5941_WriteReg(REG_AFE_ADCMAXSMEN, pCompCfg->ADCMaxHys);
}
/** @} ADC_Block_Functions */
...
void AD5941_StatisticCfgS(StatCfg_Type *pStatCfg)
{
  ...
  AD5941_WriteReg(REG_AFE_STATSCON, tempreg);
}
...
void AD5941_ADCRepeatCfgS(uint32_t Number)
{
  ...
  AD5941_WriteReg(REG_AFE_REPEATADCCNV, Number<<BITP_AFE_REPEATADCCNV_NUM);
}
...
void AD5941_DFTCfgS(DFTCfg_Type *pDftCfg)
{
  ...
  reg_adcfilter = AD5941_ReadReg(REG_AFE_ADCFILTERCON);
  if(pDftCfg->DftSrc == DFTSRC_AVG)
  {
    reg_adcfilter |= BITM_AFE_ADCFILTERCON_AVRGEN;
    AD5941_WriteReg(REG_AFE_ADCFILTERCON, reg_adcfilter);
  }
  else
  {
  ...
  AD5941_WriteReg(REG_AFE_DFTCON, reg_dftcon);
}
...
void AD5941_FIFOCfg(FIFOCfg_Type *pFifoCfg)
{
  uint32_t tempreg;
  //check parameters
  AD5941_WriteReg(REG_AFE_FIFOCON, 0);  /* Disable FIFO firstly! */
  /* CMDDATACON register. Configure this firstly */
  tempreg = AD5941_ReadReg(REG_AFE_CMDDATACON);
  ...
  /* The reset memory can be used for sequencer, configure it by function AD5941_SEQCfg() */
  AD5941_WriteReg(REG_AFE_CMDDATACON, tempreg);

  /* FIFO Threshold */
  AD5941_WriteReg(REG_AFE_DATAFIFOTHRES, pFifoCfg->FIFOThresh << BITP_AFE_DATAFIFOTHRES_HIGHTHRES);
  ...
  AD5941_WriteReg(REG_AFE_FIFOCON, tempreg);
}

/**
   @brief Read current FIFO configuration.
   @param pFifoCfg: Pointer to a buffer that used to store FIFO configuration.
   @return return AD5941ERR_OK if succeed.
*/
AD5941Err AD5941_FIFOGetCfg(FIFOCfg_Type *pFifoCfg)
{
  uint32_t tempreg;
  //check parameters
  if(pFifoCfg == NULL) return AD5941ERR_NULLP;
  /* CMDDATACON register. */
  tempreg = AD5941_ReadReg(REG_AFE_CMDDATACON);
...
/**
 * @brief Configure AD5941 FIFO Source and enable or disable FIFO.
 ...
 * @return return none.
*/
void AD5941_FIFOCtrlS(uint32_t FifoSrc, BoolFlag FifoEn)
{
  uint32_t tempreg;
...
/**
 * @brief Configure AD5941 Data FIFO threshold value
...
{
  /* FIFO Threshold */
  AD5941_WriteReg(REG_AFE_DATAFIFOTHRES, FIFOThresh << BITP_AFE_DATAFIFOTHRES_HIGHTHRES);
}
 ...
 * @return return none.
*/
uint32_t AD5941_FIFOGetCnt(void)
{
  return AD5941_ReadReg(REG_AFE_FIFOCNTSTA) >> BITP_AFE_FIFOCNTSTA_DATAFIFOCNTSTA;
}
...
void AD5941_SEQCfg(SEQCfg_Type *pSeqCfg)
{
  ...
  fifocon = AD5941_ReadReg(REG_AFE_FIFOCON);
  AD5941_WriteReg(REG_AFE_FIFOCON, 0);  /* Disable FIFO before changing memory configuration */
  ...
  {
    AD5941_WriteReg(REG_AFE_SEQCON, 0);  /* Disable sequencer firstly */
    AD5941_WriteReg(REG_AFE_SEQCNT, 0);  /* When sequencer is disabled, any write to SEQCNT will clear CNT and CRC register */  
  }
  tempreg = 0;
  if(pSeqCfg->SeqEnable == bTRUE)
    tempreg |= BITM_AFE_SEQCON_SEQEN;
  tempreg |= (pSeqCfg->SeqWrTimer) << BITP_AFE_SEQCON_SEQWRTMR;
  AD5941_WriteReg(REG_AFE_SEQCON, tempreg);
  AD5941_WriteReg(REG_AFE_FIFOCON, fifocon);  /* restore FIFO configuration */
  ...
  //   tempreg |= 0x02;  
  // AD5941_WriteReg(0x21dc, tempreg);
}
/**
 ...
 * @return return AD5941ERR_OK if succeed.
*/
AD5941Err AD5941_SEQGetCfg(SEQCfg_Type *pSeqCfg)
{
  /* check parameters */
  uint32_t tempreg;
  if(pSeqCfg == NULL)
    return AD5941ERR_NULLP;
  ...
  /* SEQCON register */
  tempreg = AD5941_ReadReg(REG_AFE_SEQCON);
...
{
  uint32_t tempreg = AD5941_ReadReg(REG_AFE_SEQCON);
  if(SeqEn == bTRUE)
    tempreg |= BITM_AFE_SEQCON_SEQEN;
  else
    tempreg &= ~BITM_AFE_SEQCON_SEQEN;
...
{
  AD5941_WriteReg(REG_AFE_SEQCON, BITM_AFE_SEQCON_SEQHALT|BITM_AFE_SEQCON_SEQEN);
}
 ...
 * @return return none.
**/
void AD5941_SEQMmrTrig(uint32_t SeqId)
{
  if(SeqId > SEQID_3)
    return;
  AD5941_WriteReg(REG_AFECON_TRIGSEQ, 1L<<SeqId);
}

/**
 * @brief Write sequencer commands to AD5941 SRAM.
 * @return return none.
**/
void AD5941_SEQCmdWrite(uint32_t StartAddr, const uint32_t *pCommand, uint32_t CmdCnt)
{
  while(CmdCnt--)
  {
    AD5941_WriteReg(REG_AFE_CMDFIFOWADDR, StartAddr++);
    AD5941_WriteReg(REG_AFE_CMDFIFOWRITE, *pCommand++);
  }
}
...
void AD5941_SEQInfoCfg(SEQInfo_Type *pSeq)
{
  switch(pSeq->SeqId)
  {
    ...
    AD5941_WriteReg(REG_AFE_SEQ0INFO, (pSeq->SeqLen<< 16) | pSeq->SeqRamAddr);
    break;
    case SEQID_1:
    AD5941_WriteReg(REG_AFE_SEQ1INFO, (pSeq->SeqLen<< 16) | pSeq->SeqRamAddr);
    break;
    case SEQID_2:
    AD5941_WriteReg(REG_AFE_SEQ2INFO, (pSeq->SeqLen<< 16) | pSeq->SeqRamAddr);
    break;
    case SEQID_3:
    AD5941_WriteReg(REG_AFE_SEQ3INFO, (pSeq->SeqLen<< 16) | pSeq->SeqRamAddr);
    break;
    default:
    break;
  }
  ...
  {
    AD5941_SEQCmdWrite(pSeq->SeqRamAddr, pSeq->pSeqCmd, pSeq->SeqLen);
  }
}
 ...
 * @param SeqId: Select from {SEQID_0, SEQID_1, SEQID_2, SEQID_3}
   ...
   @return return AD5941ERR_OK when succeed.
*/
AD5941Err AD5941_SEQInfoGet(uint32_t SeqId, SEQInfo_Type *pSeqInfo)
{
  uint32_t tempreg;
  if(pSeqInfo == NULL) return AD5941ERR_NULLP;
  switch(SeqId)
  {
    case SEQID_0:
    tempreg = AD5941_ReadReg(REG_AFE_SEQ0INFO);
    break;
    case SEQID_1:
    tempreg = AD5941_ReadReg(REG_AFE_SEQ1INFO);
    break;
    case SEQID_2:
    tempreg = AD5941_ReadReg(REG_AFE_SEQ2INFO);
    break;
    case SEQID_3:
    tempreg = AD5941_ReadReg(REG_AFE_SEQ3INFO);
    break;
    default:
			return AD5941ERR_PARA;
  }
...
/**
   ...
   @return return None.
**/
void AD5941_SEQGpioCtrlS(uint32_t Gpio)
{
  AD5941_WriteReg(REG_AFE_SYNCEXTDEVICE, Gpio);
}
...
**/
uint32_t AD5941_SEQTimeOutRd(void)
{
  return AD5941_ReadReg(REG_AFE_SEQTIMEOUT);
}
...
AD5941Err AD5941_SEQGpioTrigCfg(SeqGpioTrig_Cfg *pSeqGpioTrigCfg)
{
  ...
  if(pSeqGpioTrigCfg == NULL)
    return AD5941ERR_NULLP;
  reg_ei0con = AD5941_ReadReg(REG_ALLON_EI0CON);
  reg_ei1con = AD5941_ReadReg(REG_ALLON_EI1CON);
  ...
  AD5941_WriteReg(REG_ALLON_EI0CON, reg_ei0con);
  AD5941_WriteReg(REG_ALLON_EI0CON, reg_ei1con);
  return AD5941ERR_OK;
}
...
void AD5941_WUPTCfg(WUPTCfg_Type *pWuptCfg)
{
  ...
  AD5941_WriteReg(REG_WUPTMR_SEQ0WUPL, (pWuptCfg->SeqxWakeupTime[0] & 0xFFFF));    
  AD5941_WriteReg(REG_WUPTMR_SEQ0WUPH, (pWuptCfg->SeqxWakeupTime[0] & 0xF0000)>>16);
  AD5941_WriteReg(REG_WUPTMR_SEQ0SLEEPL, (pWuptCfg->SeqxSleepTime[0] & 0xFFFF));    
  AD5941_WriteReg(REG_WUPTMR_SEQ0SLEEPH, (pWuptCfg->SeqxSleepTime[0] & 0xF0000)>>16);

  AD5941_WriteReg(REG_WUPTMR_SEQ1WUPL, (pWuptCfg->SeqxWakeupTime[1] & 0xFFFF));    
  AD5941_WriteReg(REG_WUPTMR_SEQ1WUPH, (pWuptCfg->SeqxWakeupTime[1] & 0xF0000)>>16);
  AD5941_WriteReg(REG_WUPTMR_SEQ1SLEEPL, (pWuptCfg->SeqxSleepTime[1] & 0xFFFF));    
  AD5941_WriteReg(REG_WUPTMR_SEQ1SLEEPH, (pWuptCfg->SeqxSleepTime[1] & 0xF0000)>>16);

  AD5941_WriteReg(REG_WUPTMR_SEQ2WUPL, (pWuptCfg->SeqxWakeupTime[2] & 0xFFFF));    
  AD5941_WriteReg(REG_WUPTMR_SEQ2WUPH, (pWuptCfg->SeqxWakeupTime[2] & 0xF0000)>>16);
  AD5941_WriteReg(REG_WUPTMR_SEQ2SLEEPL, (pWuptCfg->SeqxSleepTime[2] & 0xFFFF));    
  AD5941_WriteReg(REG_WUPTMR_SEQ2SLEEPH, (pWuptCfg->SeqxSleepTime[2] & 0xF0000)>>16);

  AD5941_WriteReg(REG_WUPTMR_SEQ3WUPL, (pWuptCfg->SeqxWakeupTime[3] & 0xFFFF));    
  AD5941_WriteReg(REG_WUPTMR_SEQ3WUPH, (pWuptCfg->SeqxWakeupTime[3] & 0xF0000)>>16);
  AD5941_WriteReg(REG_WUPTMR_SEQ3SLEEPL, (pWuptCfg->SeqxSleepTime[3] & 0xFFFF));    
  AD5941_WriteReg(REG_WUPTMR_SEQ3SLEEPH, (pWuptCfg->SeqxSleepTime[3] & 0xF0000)>>16);
  
  /* TMRCON register */
  //if(pWuptCfg->WakeupEn == bTRUE)  /* enable use Wupt to wakeup AFE */
  /* We always allow Wupt to wakeup AFE automatically. */
  AD5941_WriteReg(REG_ALLON_TMRCON, BITM_ALLON_TMRCON_TMRINTEN);
  /* Wupt order */
  tempreg = 0;
  ...
  AD5941_WriteReg(REG_WUPTMR_SEQORDER, tempreg);
  ...
  //tempreg |= 1L<<4;
  AD5941_WriteReg(REG_WUPTMR_CON, tempreg);
}
 ...
 * @return return none.
*/
void AD5941_WUPTCtrl(BoolFlag Enable)
{
  uint16_t tempreg;
  tempreg = AD5941_ReadReg(REG_WUPTMR_CON);
  tempreg &= ~BITM_WUPTMR_CON_EN;
...
AD5941Err AD5941_WUPTTime(uint32_t SeqId, uint32_t SleepTime, uint32_t WakeupTime)
{
  switch (SeqId)
  {
    case SEQID_0:
    {
      AD5941_WriteReg(REG_WUPTMR_SEQ0WUPL, (WakeupTime & 0xFFFF));    
      AD5941_WriteReg(REG_WUPTMR_SEQ0WUPH, (WakeupTime & 0xF0000)>>16);
      AD5941_WriteReg(REG_WUPTMR_SEQ0SLEEPL, (SleepTime & 0xFFFF));    
      AD5941_WriteReg(REG_WUPTMR_SEQ0SLEEPH, (SleepTime & 0xF0000)>>16);
      break;
    }
    case SEQID_1:
    {
      AD5941_WriteReg(REG_WUPTMR_SEQ1WUPL, (WakeupTime & 0xFFFF));    
      AD5941_WriteReg(REG_WUPTMR_SEQ1WUPH, (WakeupTime & 0xF0000)>>16);
      AD5941_WriteReg(REG_WUPTMR_SEQ1SLEEPL, (SleepTime & 0xFFFF));    
      AD5941_WriteReg(REG_WUPTMR_SEQ1SLEEPH, (SleepTime & 0xF0000)>>16);
      break;
    }
    case SEQID_2:
    {
      AD5941_WriteReg(REG_WUPTMR_SEQ2WUPL, (WakeupTime & 0xFFFF));    
      AD5941_WriteReg(REG_WUPTMR_SEQ2WUPH, (WakeupTime & 0xF0000)>>16);
      AD5941_WriteReg(REG_WUPTMR_SEQ2SLEEPL, (SleepTime & 0xFFFF));    
      AD5941_WriteReg(REG_WUPTMR_SEQ2SLEEPH, (SleepTime & 0xF0000)>>16);
      break;
    }
    case SEQID_3:
    {
      AD5941_WriteReg(REG_WUPTMR_SEQ3WUPL, (WakeupTime & 0xFFFF));    
      AD5941_WriteReg(REG_WUPTMR_SEQ3WUPH, (WakeupTime & 0xF0000)>>16);
      AD5941_WriteReg(REG_WUPTMR_SEQ3SLEEPL, (SleepTime & 0xFFFF));    
      AD5941_WriteReg(REG_WUPTMR_SEQ3SLEEPH, (SleepTime & 0xF0000)>>16);
      break;
    }
    default:
    return AD5941ERR_PARA;
...
void AD5941_CLKCfg(CLKCfg_Type *pClkCfg)
{
  ...
  reg_osccon = AD5941_ReadReg(REG_ALLON_OSCCON);
  /* Enable clocks */
  if(pClkCfg->HFXTALEn == bTRUE)
  {
    reg_osccon |= BITM_ALLON_OSCCON_HFXTALEN;
    AD5941_WriteReg(REG_ALLON_OSCKEY,KEY_OSCCON); /* Write Key */
    AD5941_WriteReg(REG_ALLON_OSCCON, reg_osccon); /* Enable HFXTAL */
    while((AD5941_ReadReg(REG_ALLON_OSCCON)&BITM_ALLON_OSCCON_HFXTALOK) == 0); /* Wait for clock ready */
  }
  ...
  if(pClkCfg->HFOSCEn == bTRUE)
  {
    ...
    AD5941_WriteReg(REG_ALLON_OSCKEY,KEY_OSCCON); /* Write Key */
    AD5941_WriteReg(REG_ALLON_OSCCON, reg_osccon); /* Enable HFOSC */
    while((AD5941_ReadReg(REG_ALLON_OSCCON)&BITM_ALLON_OSCCON_HFOSCOK) == 0); /* Wait for clock ready */
    /* Configure HFOSC mode if it's enabled. */
    if(pClkCfg->HfOSC32MHzMode  == bTRUE)
      AD5941_HFOSC32MHzCtrl(bTRUE);
    else
      AD5941_HFOSC32MHzCtrl(bFALSE);
  }
  ...
  if(pClkCfg->LFOSCEn == bTRUE)
  {
    ...
    AD5941_WriteReg(REG_ALLON_OSCKEY,KEY_OSCCON); /* Write Key */  
    AD5941_WriteReg(REG_ALLON_OSCCON, reg_osccon); /* Enable LFOSC */
    while((AD5941_ReadReg(REG_ALLON_OSCCON)&BITM_ALLON_OSCCON_LFOSCOK) == 0); /* Wait for clock ready */
  }
  ...
  if(pClkCfg->LFOSCEn == bFALSE)
    reg_osccon &= ~BITM_ALLON_OSCCON_LFOSCEN;
  AD5941_WriteReg(REG_ALLON_OSCKEY, KEY_OSCCON); /* Write Key */
  AD5941_WriteReg(REG_ALLON_OSCCON, reg_osccon);
}
...
void AD5941_HFOSC32MHzCtrl(BoolFlag Mode32MHz)
{
  ...
  RdCLKEN1 = AD5941_ReadReg(REG_AFECON_CLKEN1);
  
  SPI_Para.SPI_RW_DATA_32 = RdCLKEN1;
  ...
  AD5941_WriteReg(REG_AFECON_CLKEN1,RdCLKEN1|BITM_AFECON_CLKEN1_ACLKDIS); /* Disable ACLK during clock changing */

  RdHPOSCCON = AD5941_ReadReg(REG_AFE_HPOSCCON);  // 0x20BC - 0x80000000 ?
  
  SPI_Para.SPI_RW_DATA_32 = RdHPOSCCON;
  
  if(Mode32MHz == bTRUE)
  {
    AD5941_WriteReg(REG_AFE_HPOSCCON,RdHPOSCCON&(~BITM_AFE_HPOSCCON_CLK32MHZEN)); /* Enable 32MHz output(bit definition-0: 32MHz, 1: 16MHz) */  
    while((AD5941_ReadReg(REG_ALLON_OSCCON)&BITM_ALLON_OSCCON_HFOSCOK) == 0); /* Wait for clock ready */
  }
  else
  {
    AD5941_WriteReg(REG_AFE_HPOSCCON,RdHPOSCCON|BITM_AFE_HPOSCCON_CLK32MHZEN); /* Enable 16MHz output(bit definition-0: 32MHz, 1: 16MHz) */       
    while((AD5941_ReadReg(REG_ALLON_OSCCON)&BITM_ALLON_OSCCON_HFOSCOK) == 0); /* Wait for clock ready */
  }

  AD5941_WriteReg(REG_AFECON_CLKEN1,RdCLKEN1&(~BITM_AFECON_CLKEN1_ACLKDIS)); /* Enable ACLK */
}
/**
 ...
 * @return return none.
*/
void 			AD5941_HPModeEn(BoolFlag Enable)
{
	CLKCfg_Type clk_cfg;
	uint32_t temp_reg = 0;
	
	/* Check what the system clock is */
	temp_reg = AD5941_ReadReg(REG_AFECON_CLKSEL);
  ...
  clk_cfg.SysClkSrc = temp_reg & 0x3; 
	if(Enable == bTRUE)
	{
		...
		AD5941_AFEPwrBW(AFEPWR_HP, AFEBW_250KHZ);
	}
	else
	{
		clk_cfg.SysClkDiv = SYSCLKDIV_1;
		clk_cfg.HfOSC32MHzMode = bFALSE;
		AD5941_AFEPwrBW(AFEPWR_LP, AFEBW_100KHZ);
	}
 ...
 *       - AFEINTSRC_CRC_OUTLIER  : Bit29, CRC interrupt for M355, Outliers Int for AD5941  
 *       - AFEINTSRC_GPT0INT_SLPWUT: Bit30, General Purpose Timer0 IRQ for M355. Sleep or Wakeup Timer timeout for AD5941
 *       - AFEINTSRC_GPT1INT_TRYBRK: Bit31, General Purpose Timer1 IRQ for M355. Tried to Break IRQ for AD5941
...
uint32_t AD5941_INTCGetCfg(uint32_t AfeIntcSel)
{
  ...
  if(AfeIntcSel == AFEINTC_0)
    tempreg = AD5941_ReadReg(REG_INTC_INTCSEL0);
  else
    tempreg = AD5941_ReadReg(REG_INTC_INTCSEL1);
  return tempreg;
}
...
{
  AD5941_WriteReg(REG_INTC_INTCCLR,AfeIntSrcSel);
}
...
**/
BoolFlag AD5941_INTCTestFlag(uint32_t AfeIntcSel, uint32_t AfeIntSrcSel)
{
  uint32_t tempreg;
...
**/
uint32_t AD5941_INTCGetFlag(uint32_t AfeIntcSel)
{
  uint32_t tempreg;
  uint32_t regaddr = (AfeIntcSel == AFEINTC_0)? REG_INTC_INTCFLAG0: REG_INTC_INTCFLAG1;
...
{
  AD5941_AGPIOFuncCfg(pAgpioCfg->FuncSet);
  AD5941_AGPIOOen(pAgpioCfg->OutputEnSet);
  AD5941_AGPIOIen(pAgpioCfg->InputEnSet);
  AD5941_AGPIOPen(pAgpioCfg->PullEnSet);
  AD5941_WriteReg(REG_AGPIO_GP0OUT, pAgpioCfg->OutVal);
}
...
{
   AD5941_WriteReg(REG_AGPIO_GP0CON,uiCfgSet);
}
...
{
   AD5941_WriteReg(REG_AGPIO_GP0OEN,uiPinSet);
}
...
{
   AD5941_WriteReg(REG_AGPIO_GP0IEN,uiPinSet);
}
...
{
  return AD5941_ReadReg(REG_AGPIO_GP0IN);
}
...
{
   AD5941_WriteReg(REG_AGPIO_GP0PE,uiPinSet);
}
...
{
   AD5941_WriteReg(REG_AGPIO_GP0SET,uiPinSet);
}
...
{
   AD5941_WriteReg(REG_AGPIO_GP0CLR,uiPinSet);
}
...
{
   AD5941_WriteReg(REG_AGPIO_GP0TGL,uiPinSet);
}
 ...
 *          Control most blocks with in one register. The so called LPMODE has nothing to do with AD5941 power.
 * @return return AD5941ERR_OK
**/
AD5941Err AD5941_LPModeEnS(BoolFlag LPModeEn)
{
  if(LPModeEn == bTRUE)
    AD5941_WriteReg(REG_AFE_LPMODEKEY, KEY_LPMODEKEY);  /* Enter LP mode by right key. */
  else
    AD5941_WriteReg(REG_AFE_LPMODEKEY, 0); /* Write wrong key to exit LP mode */
  return AD5941ERR_OK;
}

/**
 * @brief Select system clock source for LPMODE. 
 * @note Only in LP Mode, this operation takes effect. Enter LPMODE by function @ref AD5941_LPModeEnS.
...
{
  AD5941_WriteReg(REG_AFE_LPMODECLKSEL, LPModeClk);
}
 ...
 * @param SlpKey : {SLPKEY_UNLOCK, SLPKEY_LOCK}
          ...
          - SLPKEY_LOCK Lock key so AD5941 is prohibited to enter sleep mode.
   @return return none.
*/
void AD5941_SleepKeyCtrlS(uint32_t SlpKey)
{
  AD5941_WriteReg(REG_AFE_SEQSLPLOCK, SlpKey);
}

/**
 * @brief Put AFE to hibernate. 
 * @details This will only take effect when SLP_KEY has been unlocked. Use function @ref AD5941_SleepKeyCtrlS to enter correct key.
 * @return return none.
*/
void AD5941_EnterSleepS(void)
{
  AD5941_WriteReg(REG_AFE_SEQTRGSLP, 0);
  AD5941_WriteReg(REG_AFE_SEQTRGSLP, 1);
}

/**
 * @brief Turn off LP-Loop and put AFE to hibernate mode;
 * @details By function @ref AD5941_EnterSleepS, we can put most blocks to hibernate mode except LP block.
 *          This function will shut down LP block and then enter sleep mode.
 * @return return none.
*/
void AD5941_ShutDownS(void)
{
  ...
  AD5941_StructInit(&aferef_cfg, sizeof(aferef_cfg));
  AD5941_StructInit(&lp_loop, sizeof(lp_loop));
  AD5941_REFCfgS(&aferef_cfg);
  AD5941_LPLoopCfgS(&lp_loop);
  AD5941_SleepKeyCtrlS(SLPKEY_UNLOCK);  /* Unlock the key */
  AD5941_EnterSleepS();  /* Enter Hibernate */
}

/**
 * @brief Try to wakeup AD5941 by read register.
 * @details Any SPI operation can wakeup AD5941. AD5941_Initialize must be called to enable this function.
...
*/
uint32_t  AD5941_WakeUp(int32_t TryCount)
{
  uint32_t count = 0;
  while(1)
  {
    count++;
    if(AD5941_ReadReg(REG_AFECON_ADIID) == AD5941_ADIID)
      break;    /* Succeed */
    if(TryCount<=0) 
...
/**
 * @brief Read ADIID register, the value for current version is @ref AD5941_ADIID
 * @return return none.
*/
uint32_t AD5941_GetADIID(void)
{
  return AD5941_ReadReg(REG_AFECON_ADIID);
}
 ...
 * @return return none.
*/
uint32_t AD5941_GetChipID(void)
{
  return AD5941_ReadReg(REG_AFECON_CHIPID);
}
/**
 * @brief Reset AD5941 by register.
 * @note AD5941 must be in active state so we can access registers.
 *       If AD5941 system clock is too low, we consider to use hardware reset, or 
 *       we need to make sure register write is successfully.
 * @return return none.
*/
AD5941Err  AD5941_SoftRst(void)
{
  AD5941_WriteReg(REG_AFECON_SWRSTCON, AD5941_SWRST);
  AD5941_Delay10us(20); /* AD5941 need some time to exit reset status. 200us looks good. */
  /* We can check RSTSTA register to make sure software reset happened. */
  return AD5941ERR_OK;
}

/**
 * @brief Reset AD5941 with RESET pin.
 * @note This will call function AD5941_RstClr which locates in file XXXPort.C
 * @return return none.
*/
void AD5941_HWReset(void)
{
#ifndef CHIPSEL_M355
  AD5941_RstClr();
  AD5941_Delay10us(200); /* Delay some time */
  AD5941_RstSet();
  AD5941_Delay10us(500); /* AD5941 need some time to exit reset status. 200us looks good. */
#else
 ...
 * @return return none.
*/
static void __AD5941_ReferenceON(void)
{
...
*/
static uint32_t __AD5941_TakeMeasurement(int32_t *time_out)
{
  uint32_t ADCCode = 0;
  AD5941_INTCClrFlag(AFEINTSRC_SINC2RDY);
  AD5941_AFECtrlS(AFECTRL_ADCCNV|AFECTRL_SINC2NOTCH, bTRUE);/* Start conversion */
  do
  {
    AD5941_Delay10us(1);  /* Delay 10us */
    if(AD5941_INTCTestFlag(AFEINTC_1,AFEINTSRC_SINC2RDY))
    {
        ADCCode = AD5941_ReadAfeResult(AFERESULT_SINC2);
        break;
    }
    if(*time_out != -1)
      (*time_out)--;	
  }while(*time_out != 0);
  AD5941_AFECtrlS(AFECTRL_ADCCNV|AFECTRL_SINC2NOTCH, bFALSE);/* Stop conversion */
  return ADCCode;
}
...
/**
  ...
  @return AD5941ERR_OK.
**/
AD5941Err AD5941_ADCPGACal(ADCPGACal_Type *pADCPGACal)
{
  ...
  if(pADCPGACal == NULL) return AD5941ERR_NULLP;
  if(pADCPGACal->ADCPga > ADCPGA_9) return AD5941ERR_PARA;  /* Parameter Error */
  ...
  adc_filter.Sinc2NotchEnable = bTRUE;        /* Enable the SINC2+Notch block. You can also use function AD5941_AFECtrlS */
  AD5941_ADCFilterCfgS(&adc_filter);
  ...
  AD5941_AFECtrlS(AFECTRL_ALL, bFALSE); /* Disable all */
  AD5941_AFECtrlS(AFECTRL_ADCPWR|AFECTRL_HPREFPWR|AFECTRL_DACREFPWR|AFECTRL_HSDACPWR|AFECTRL_SINC2NOTCH, bTRUE);
  AD5941_Delay10us(25);   /* Wait 250us for reference power up */
  /* INTC configure and open calibration lock */
  INTCCfg = AD5941_INTCGetCfg(AFEINTC_1);
  AD5941_INTCCfg(AFEINTC_1, AFEINTSRC_SINC2RDY, bTRUE); /* Enable SINC2 Interrupt in INTC1 */
  AD5941_WriteReg(REG_AFE_CALDATLOCK, KEY_CALDATLOCK);  /* Unlock KEY */

  /* Do offset calibration. */
  if(pADCPGACal->PGACalType != PGACALTYPE_GAIN){  /* Need offset calibration */
    int32_t ExpectedCode = 0x8000;        /* Ideal ADC output */
    AD5941_WriteReg(regaddr_offset, 0);   /* Reset offset register */
    ...
    AD5941_ADCBaseCfgS(&adc_base);
    AD5941_Delay10us(5);                  /* Wait for sometime */
    ADCCode = 0;
    for(int i=0; i<8; i++)
    { /* ADC offset calibration register has resolution of 0.25LSB. take full use of it. */
      ...
      ADCCode += __AD5941_TakeMeasurement(&time_out);  /* Turn on ADC to get one valid data and then turn off ADC. */
  ...
  if(pADCPGACal->PGACalType != PGACALTYPE_OFFSET)  /* Need gain calibration */
  {
    ...
    AD5941_WriteReg(regaddr_gain, 0x4000);  /* Reset gain register */
    ...
    else
    {
      ...
      AD5941_AFECtrlS(AFECTRL_EXTBUFPWR|AFECTRL_INAMPPWR|AFECTRL_HSTIAPWR|AFECTRL_WG, bTRUE);
      ...
      AD5941_Delay10us(5);
      time_out = pADCPGACal->TimeOut10us;   /* Reset time out counter */
      ExpectedGainCode = 0x8000 + (int32_t)((__AD5941_TakeMeasurement(&time_out) - 0x8000)/1.5f\
    ...
    AD5941_Delay10us(5);
    time_out = pADCPGACal->TimeOut10us;      /* Reset time out counter */
    ADCCode = __AD5941_TakeMeasurement(&time_out);
    if(time_out == 0) goto ADCPGACALERROR_TIMEOUT;
    /* Calculate and write the result to registers */
    ADCCode = (ExpectedGainCode - 0x8000)*0x4000/(ADCCode-0x8000);
    ADCCode &= 0x7fff;
    AD5941_WriteReg(regaddr_gain, ADCCode);
  }

  /* Restore INTC1 SINC2 configure */
  if(INTCCfg&AFEINTSRC_SINC2RDY);
  else
    AD5941_INTCCfg(AFEINTC_1, AFEINTSRC_SINC2RDY, bFALSE); /* Disable SINC2 Interrupt */

  AD5941_WriteReg(REG_AFE_CALDATLOCK, 0);  /* Lock KEY */
  /* Done */
  return AD5941ERR_OK;

ADCPGACALERROR_TIMEOUT:
  AD5941_ADCConvtCtrlS(bFALSE);  /* Stop conversion */
  AD5941_WriteReg(REG_AFE_CALDATLOCK, 0);  /* Lock KEY */
  return AD5941ERR_TIMEOUT;
}
...
AD5941Err AD5941_LPTIAOffsetCal(LPTIAOffsetCal_Type *pLPTIAOffsetCal)
{
  ...
  if(pLPTIAOffsetCal == NULL) return AD5941ERR_NULLP;  
  if(pLPTIAOffsetCal->AdcClkFreq > (32000000*0.8))
    bADCClk32MHzMode = bTRUE;

  /* Step0: Do initialization */
  /* Turn on AD5941 references in case it's disabled. */
  __AD5941_ReferenceON();
  ...
  AD5941_AFECtrlS(AFECTRL_ALL, bFALSE); /* Disable all firstly, we only enable things we use */
  AD5941_AFECtrlS(AFECTRL_ADCPWR|AFECTRL_HPREFPWR|AFECTRL_SINC2NOTCH, bTRUE);
  AD5941_Delay10us(25);                     /* Wait 250us for reference power up */
  /* INTC configure and open calibration lock */
  INTCCfg = AD5941_INTCGetCfg(AFEINTC_1);
  AD5941_INTCCfg(AFEINTC_1, AFEINTSRC_SINC2RDY, bTRUE); /* Enable SINC2 Interrupt in INTC1 */
  ...
  {
    int32_t ExpectedCode = 0x8000;        /* Ideal ADC output */
    AD5941_WriteReg(REG_AFE_ADCOFFSETLPTIA0, 0);   /* Reset offset register */

    if(pLPTIAOffsetCal->SettleTime10us > 0)
      AD5941_Delay10us(pLPTIAOffsetCal->SettleTime10us);  /* Delay 10us */
    time_out = pLPTIAOffsetCal->TimeOut10us;   /* Reset time out counter */
    ADCCode = __AD5941_TakeMeasurement(&time_out);  /* Turn on ADC to get one valid data and then turn off ADC. */
    if(time_out == 0)
    {
      error = AD5941ERR_TIMEOUT;
    ...
    {
      error = AD5941ERR_CALOR;
      goto LPTIAOFFSETCALERROR;
    }
    ADCCode &= 0x7fff;
    if(pLPTIAOffsetCal->LpAmpSel == LPAMP0)
      AD5941_WriteReg(REG_AFE_ADCOFFSETLPTIA0, ADCCode);
    else
      AD5941_WriteReg(REG_AFE_ADCOFFSETLPTIA1, ADCCode);
  }
  /* Restore INTC1 SINC2 configure */
  if(INTCCfg&AFEINTSRC_SINC2RDY);
  else
 ...
 * @return AD5941ERR_OK.
**/
AD5941Err AD5941_HSTIAOffsetCal(LPTIAOffsetCal_Type *pHSTIAOffsetCal)
{
  return AD5941ERR_OK;
}
 ...
 * @return AD5941ERR_OK if succeed.
**/
AD5941Err AD5941_HSRtiaCal(HSRTIACal_Type *pCalCfg, void *pResult)
{
  ...
  if(pCalCfg == NULL) return AD5941ERR_NULLP;
  if(pCalCfg->fRcal == 0)
    return AD5941ERR_PARA;
  if(pCalCfg->HsTiaCfg.HstiaRtiaSel > HSTIARTIA_160K)
    return AD5941ERR_PARA;
  if(pCalCfg->HsTiaCfg.HstiaRtiaSel == HSTIARTIA_OPEN)
    return AD5941ERR_PARA; /* Do not support calibrating DE0-RTIA */
  if(pResult == NULL)
      return AD5941ERR_NULLP;
  ...
  */
  ...
  /*INTC configuration */
  INTCCfg = AD5941_INTCGetCfg(AFEINTC_1);
  AD5941_INTCCfg(AFEINTC_1, AFEINTSRC_DFTRDY, bTRUE); /* Enable SINC2 Interrupt in INTC1 */
  
  AD5941_AFECtrlS(AFECTRL_ALL, bFALSE);  /* Init all to disable state */
  ...
  AD5941_DSPCfgS(&dsp_cfg);

  /* Enable all of them. They are automatically turned off during hibernate mode to save power */
  AD5941_AFECtrlS(AFECTRL_HSTIAPWR|AFECTRL_INAMPPWR|AFECTRL_EXTBUFPWR|\
                /*AFECTRL_WG|*/AFECTRL_DACREFPWR|AFECTRL_HSDACPWR|\
                AFECTRL_SINC2NOTCH, bTRUE);
  
  AD5941_AFECtrlS(AFECTRL_WG|AFECTRL_ADCPWR, bTRUE);  /* Enable Waveform generator, ADC power */
  //wait for sometime.
  AD5941_Delay10us(25);
  AD5941_AFECtrlS(AFECTRL_ADCCNV|AFECTRL_DFT, bTRUE);  /* Start ADC convert and DFT */
  /* Wait until DFT ready */
  while(AD5941_INTCTestFlag(AFEINTC_1, AFEINTSRC_DFTRDY) == bFALSE);  
  AD5941_AFECtrlS(AFECTRL_ADCCNV|AFECTRL_DFT|AFECTRL_WG|AFECTRL_ADCPWR, bFALSE);  /* Stop ADC convert and DFT */
  AD5941_INTCClrFlag(AFEINTSRC_DFTRDY);
  
  DftRcal.Real = AD5941_ReadAfeResult(AFERESULT_DFTREAL);
  DftRcal.Image = AD5941_ReadAfeResult(AFERESULT_DFTIMAGE);

  AD5941_ADCMuxCfgS(ADCMUXP_HSTIA_P, ADCMUXN_HSTIA_N);
  AD5941_AFECtrlS(AFECTRL_WG|AFECTRL_ADCPWR, bTRUE);  /* Enable Waveform generator, ADC power */
  //wait for sometime.
  AD5941_Delay10us(25);
  AD5941_AFECtrlS(AFECTRL_ADCCNV|AFECTRL_DFT, bTRUE);  /* Start ADC convert and DFT */
  ...
  DftRtia.Real = AD5941_ReadAfeResult(AFERESULT_DFTREAL);
  DftRtia.Image = AD5941_ReadAfeResult(AFERESULT_DFTIMAGE);
  ...
  */
  ...
  fImpCar_Type temp;
  temp = AD5941_ComplexDivInt(&DftRtia, &DftRcal);
  ...
  {
    ((fImpPol_Type*)pResult)->Magnitude = AD5941_ComplexMag(&temp);
    ((fImpPol_Type*)pResult)->Phase = AD5941_ComplexPhase(&temp);
  }
  
  /* Restore INTC1 DFT configure */
  if(INTCCfg&AFEINTSRC_DFTRDY);
  else
    AD5941_INTCCfg(AFEINTC_1, AFEINTSRC_DFTRDY, bFALSE); /* Disable DFT Interrupt */
...
AD5941Err AD5941_LPRtiaCal(LPRTIACal_Type *pCalCfg, void *pResult)
{
  ...
  if(pCalCfg == NULL)  return AD5941ERR_NULLP;  /* Parameters illegal */
  ...
  if(pCalCfg->LpTiaRtia == LPTIARTIA_OPEN)
    return AD5941ERR_PARA; /* Not supported now. By setting RTIA to open and set corresponding switches can calibrate external RTIA */
  if(pResult == NULL)
      return AD5941ERR_NULLP;
 ...
 /** @cond */
  #define MAXVOLT_P2P 1400  /* Maximum peak to peak voltage 1200mV for ADuCM355. */  
                            /* Maximum peak2peak voltage for AD5941 10kOhm RCAL is 1400mV */
  ...
  */
  ...
  reg_afecon = AD5941_ReadReg(REG_AFE_AFECON);
  /* INTC configuration */
  INTCCfg = AD5941_INTCGetCfg(AFEINTC_1);
  AD5941_INTCCfg(AFEINTC_1, AFEINTSRC_DFTRDY|AFEINTSRC_SINC2RDY, bTRUE); /* Enable SINC2 Interrupt in INTC1 */
  ...
  /* Configure reference system */
	__AD5941_ReferenceON();
	/* Configure DSP */
	AD5941_StructInit(&dsp_cfg, sizeof(dsp_cfg));
	dsp_cfg.ADCFilterCfg.ADCAvgNum = ADCAVGNUM_16;  /* Don't care because it's disabled */
	dsp_cfg.ADCFilterCfg.ADCRate = bADCClk32MHzMode?ADCRATE_1P6MHZ:ADCRATE_800KHZ;
	dsp_cfg.ADCFilterCfg.ADCSinc2Osr = pCalCfg->ADCSinc2Osr;
	dsp_cfg.ADCFilterCfg.ADCSinc3Osr = pCalCfg->ADCSinc3Osr;
  ...
  AD5941_LPLoopCfgS(&lp_loop);
  /* Configure HS Loop */
  AD5941_StructInit(&hs_loop, sizeof(hs_loop));
  ...
  if(bDCMode)
  {
    ...
    AD5941_HSLoopCfgS(&hs_loop);
    AD5941_WGDACCodeS(WgAmpWord + 0x800);
		AD5941_AFECtrlS(AFECTRL_HSTIAPWR|AFECTRL_INAMPPWR|AFECTRL_WG|AFECTRL_ADCPWR, bTRUE); /* Apply voltage to loop and turn on ADC */
    ...
    AD5941_SWMatrixCfgS(pSWCfg);    
    AD5941_Delay10us(1000);   /* Wait some time here. */
    ...
    AD5941_ADCBaseCfgS(pADCBaseCfg);
    AD5941_Delay10us(50);   /* Wait some time here. */
    offset_rcal = __AD5941_TakeMeasurement(&time_out);  /* Turn on ADC to get one valid data and then turn off ADC. */
    ...
    AD5941_ADCBaseCfgS(pADCBaseCfg);
    AD5941_Delay10us(50);   /* Wait some time here. */
    offset_rtia = __AD5941_TakeMeasurement(&time_out);  /* Turn on ADC to get one valid data and then turn off ADC. */
    ...
    AD5941_ADCBaseCfgS(pADCBaseCfg);
    AD5941_Delay10us(50);   /* Wait some time here. */
    DftRcal.Real = (int32_t)__AD5941_TakeMeasurement(&time_out)- offset_rcal;
    DftRcal.Image = 0;
		/* Measure RTIA */    
    if(pCalCfg->LpAmpSel == LPAMP0)
    {
    ...
    AD5941_ADCBaseCfgS(pADCBaseCfg);
    AD5941_Delay10us(50);   /* Wait some time here. */
    DftRtia.Real = (int32_t)__AD5941_TakeMeasurement(&time_out)- offset_rtia;
    DftRtia.Image = 0;
  }
  else
  {
    ...
    AD5941_AFECtrlS(AFECTRL_HSTIAPWR|AFECTRL_INAMPPWR, bTRUE);
    AD5941_Delay10us(100);      /* Wait for loop stable. */
    pADCBaseCfg = &dsp_cfg.ADCBaseCfg;
		/* DFT on RCAL */
    pADCBaseCfg->ADCMuxN = ADCMUXN_N_NODE;
    pADCBaseCfg->ADCMuxP = ADCMUXP_P_NODE;
    pADCBaseCfg->ADCPga = ADCPgaGainRcal;
    AD5941_ADCBaseCfgS(pADCBaseCfg);
    AD5941_AFECtrlS(AFECTRL_ADCPWR|AFECTRL_WG, bTRUE);
    AD5941_Delay10us(25);
    AD5941_AFECtrlS(AFECTRL_ADCCNV|AFECTRL_DFT, bTRUE);
    /* Wait until DFT ready */
    while(AD5941_INTCTestFlag(AFEINTC_1, AFEINTSRC_DFTRDY) == bFALSE);  
    AD5941_AFECtrlS(AFECTRL_ADCCNV|AFECTRL_DFT|AFECTRL_WG|AFECTRL_ADCPWR, bFALSE);  /* Stop ADC convert and DFT */
    AD5941_INTCClrFlag(AFEINTSRC_DFTRDY);
  ...
  */
  ...
  res = AD5941_ComplexDivInt(&DftRtia, &DftRcal);
  ...
  {
    ((fImpPol_Type*)pResult)->Magnitude = AD5941_ComplexMag(&res);
    ((fImpPol_Type*)pResult)->Phase = AD5941_ComplexPhase(&res);
  }
  ...
  if(INTCCfg&AFEINTSRC_SINC2RDY);
    ...
    AD5941_INTCCfg(AFEINTC_1, AFEINTSRC_SINC2RDY, bFALSE);  /* Disable SINC2 Interrupt */
  AD5941_WriteReg(REG_AFE_AFECON, reg_afecon);              /* Restore AFECON register */
  ...
  AD5941_SWMatrixCfgS(&hs_loop.SWMatCfg);
 ...
 * @return return AD5941ERR_OK if succeeded.
*/
AD5941Err AD5941_HSDACCal(HSDACCal_Type *pCalCfg)
{
  ...
  if(pCalCfg == NULL) return AD5941ERR_NULLP;
  if(pCalCfg->ExcitBufGain > 1) return AD5941ERR_PARA;
  if(pCalCfg->HsDacGain > 1) return AD5941ERR_PARA;
  ...
  AD5941_AFECtrlS(AFECTRL_ALL, bFALSE); /* Disable all */
  AD5941_AFECtrlS(AFECTRL_ADCPWR|AFECTRL_HPREFPWR|AFECTRL_DACREFPWR|AFECTRL_HSDACPWR|AFECTRL_SINC2NOTCH|\
  ...
  AD5941_WriteReg(REG_AFE_CALDATLOCK, KEY_CALDATLOCK);  /* Unlock KEY */
	...
	AD5941_WriteReg(regaddr_offset, 0);
	/* Update HSDACDAT after resetting calibration register */
	AD5941_WriteReg(REG_AFE_HSDACDAT, 0x800);
  /* Step1: Do offset calibration. */
  {
    ...
    ADCCode = __AD5941_TakeMeasurement(&time_out);
#ifdef ADI_DEBUG
    ADI_Print("Voltage before cal: %f \n", AD5941_ADCCode2Volt(ADCCode, ADCPGA_Sel, 1.82));
#endif
...
AD5941Err AD5941_LPDACCal(LPDACCal_Type *pCalCfg, LPDACPara_Type *pResult)
{
  ...
  if(pCalCfg == NULL) return AD5941ERR_NULLP; 
  if(pResult == NULL) return AD5941ERR_NULLP;  
  if(pCalCfg->AdcClkFreq > (32000000*0.8))
    bADCClk32MHzMode = bTRUE;
  ...
  {
    //AD5941_ADCMuxCfgS(ADCMUXP_AGND, ADCMUXN_VSET1P1);
    time_out = pCalCfg->TimeOut10us;   /* Reset time out counter */
    ADCCodeVref1p1 = __AD5941_TakeMeasurement(&time_out);  /* Turn on ADC to get one valid data and then turn off ADC. */
    if(time_out == 0)
    {
      error = AD5941ERR_TIMEOUT;
      goto LPDACCALERROR;
  ...
  /* Step2: Do offset measurement. */
  {
    ...
    AD5941_LPDACWriteS(0,0);  /* Set LPDAC output voltage to 0.2V(zero code) */
    if(pCalCfg->SettleTime10us > 0)
      AD5941_Delay10us(pCalCfg->SettleTime10us);  /* Delay nx10us */
    if(pCalCfg->LpdacSel == LPDAC0)
      AD5941_ADCMuxCfgS(ADCMUXP_VBIAS0, ADCMUXN_VREF1P1); /* Vbias0 is routed to 12BIT LPDAC */
    else
      AD5941_ADCMuxCfgS(ADCMUXP_VBIAS1, ADCMUXN_VREF1P1); /* Vbias1 is routed to 12BIT LPDAC */

    AD5941_Delay10us(5);  /* Delay 50us */
    time_out = pCalCfg->TimeOut10us;   /* Reset time out counter */
    ADCCode = __AD5941_TakeMeasurement(&time_out);  /* Turn on ADC to get one valid data and then turn off ADC. */
    if(time_out == 0)
    {
    ...
    if(pCalCfg->LpdacSel == LPDAC0)
      AD5941_ADCMuxCfgS(ADCMUXP_VZERO0, ADCMUXN_VREF1P1); /* Vbias0 is routed to 12BIT LPDAC */
    else
      AD5941_ADCMuxCfgS(ADCMUXP_VZERO1, ADCMUXN_VREF1P1); /* Vbias1 is routed to 12BIT LPDAC */
    AD5941_Delay10us(5);  /* Delay 50us */
    time_out = pCalCfg->TimeOut10us;   /* Reset time out counter */
    ADCCode = __AD5941_TakeMeasurement(&time_out);  /* Turn on ADC to get one valid data and then turn off ADC. */
    if(time_out == 0)
  ...
  /* Step3: Do gain measurement */
  {
    ...
    AD5941_LPDACWriteS(0xfff,0x3f);  /* Set LPDAC output voltage to 2.4V(zero code) */
    if(pCalCfg->SettleTime10us > 0)
      AD5941_Delay10us(pCalCfg->SettleTime10us);  /* Delay nx10us */
    if(pCalCfg->LpdacSel == LPDAC0)
      AD5941_ADCMuxCfgS(ADCMUXP_VBIAS0, ADCMUXN_VREF1P1); /* Vbias0 is routed to 12BIT LPDAC */
 ...
 * @return AD5941ERR_OK if succeed.
**/
AD5941Err AD5941_LFOSCMeasure(LFOSCMeasure_Type *pCfg, float *pFreq) /* Measure current LFOSC frequency. */
{
  /**
   * @code
  ...
  if(pCfg == NULL) return AD5941ERR_NULLP;
  if(pFreq == NULL) return AD5941ERR_NULLP;
  if(pCfg->CalDuration < 1.0f)
    return AD5941ERR_PARA;
  AD5941_SEQGetCfg(&seq_cfg_backup);
  INTCCfg = AD5941_INTCGetCfg(AFEINTC_1);
  AD5941_INTCCfg(AFEINTC_1, AFEINTSRC_ENDSEQ, bTRUE);
	AD5941_INTCClrFlag(AFEINTSRC_ALLINT);
  ...
  AD5941_SEQCfg(&seq_cfg);          /* Enable sequencer */
  ...
  AD5941_WUPTCfg(&wupt_cfg);
  
  AD5941_INTCClrFlag(AFEINTSRC_ENDSEQ);
  AD5941_WUPTCtrl(bTRUE);
  
  while(AD5941_INTCTestFlag(AFEINTC_1, AFEINTSRC_ENDSEQ) == bFALSE);
  TimerCount = AD5941_SEQTimeOutRd();
  
  AD5941_WUPTCtrl(bFALSE);
	AD5941_WUPTTime(SEQID_0, 4, 4);	/* Set it to minimum value because we don't care about sequence0 now. We only want to measure how much time MCU will need to read register */
  seqinfo.SeqId = SEQID_1;
  seqinfo.SeqRamAddr = pCfg->CalSeqAddr + SEQ_LEN(SeqA) ;
  ...
  seqinfo.WriteSRAM = bTRUE;
  AD5941_SEQInfoCfg(&seqinfo);
  AD5941_SEQCtrlS(bTRUE); /* Enable Sequencer again */
  ...
  AD5941_WUPTCtrl(bTRUE);
  while(AD5941_INTCTestFlag(AFEINTC_1, AFEINTSRC_ENDSEQ) == bFALSE);
  TimerCount2 = AD5941_SEQTimeOutRd();
	AD5941_INTCTestFlag(AFEINTC_0, AFEINTSRC_ENDSEQ);

  AD5941_WUPTCtrl(bFALSE);
  AD5941_SEQCfg(&seq_cfg_backup);          /* restore sequencer configuration */
  AD5941_INTCCfg(AFEINTC_1, AFEINTSRC_ENDSEQ, (INTCCfg&AFEINTSRC_ENDSEQ)?bTRUE:bFALSE); /* Restore interrupt configuration */
  AD5941_INTCClrFlag(AFEINTSRC_ENDSEQ);
...
{
//  if(EXTI_GetITStatus(FK_EXTI5) != RESET) 
//  {
//    ucInterrupted = 1;
//  }
//  EXTI_ClearITPendingBit(FK_EXTI5);
...
void AD5941_ReadWriteNBytes(unsigned char *pSendBuffer,unsigned char *pRecvBuff,unsigned long length)
{
  ...
  /* Software transfer mode, still sorking on it. */
  /*
  // SPI Initialize
  // FK_SPI1_ENABLE_API(pSendBuffer, 1);
  
  for(unsigned long i = 0; i<length; i++)
  {
    temp=*pSendBuffer;
    
    SPI_Para.SPI_TX[i] = temp;
  ...
  */
  ...
  /* Hardware transfer mode */
  SPI_Para.SPIRWtimes++;
  
    // Send/Read also record the TX data
  for(unsigned long i=0; i<length; i++)
  {
    temp=*pSendBuffer;
    SPI_Para.SPI_TX[i] = temp;   
  ...
  // Clear the data inside buffer
//  for(int j=0; j<sizeof(SPI_Para.SPI_TX); j++)
//  {
//    SPI_Para.SPI_TX[j] = 0;
//    SPI_Para.SPI_RX[j] = 0;
//  }
