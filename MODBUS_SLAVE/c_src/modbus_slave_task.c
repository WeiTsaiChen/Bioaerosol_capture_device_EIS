...
// Copyright (C) 2009, FINE-TEK CO.,LTD. Taiwan  All Rights Reserved
//
//! @fn 	     modbus_slave_write_variable_value
//
//! @author 	 Willman Chen
//
//! @date 		 2009/10/26
...
// Copyright (C) 2009, FINE-TEK CO.,LTD. Taiwan  All Rights Reserved
//
//! @fn 	 modbus_slave_get_variable_pointer
//
//! @author 	 Willman Chen
//
//! @date 	 2009/10/26
...
{
    UINT16 *data_ptr;
    if (register_addr>=MODBUS_SLAVE_HEADER_START_ADDRESS && register_addr<MODBUS_SLAVE_READONLY_START_ADDRESS)
    {
        data_ptr=(UINT16*)gt_modbus_slave_header[register_addr-MODBUS_SLAVE_HEADER_START_ADDRESS];
    }
    else if (register_addr>=MODBUS_SLAVE_READONLY_START_ADDRESS && register_addr<MODBUS_SLAVE_READ_WRITE_START_ADDRESS)
    {
        data_ptr=(UINT16*)gt_modbus_slave_readonly[register_addr-MODBUS_SLAVE_READONLY_START_ADDRESS];
    }
   else if (register_addr>=MODBUS_SLAVE_READ_WRITE_START_ADDRESS && register_addr<MODBUS_SLAVE_DDF_START_ADDRESS)
   {
        data_ptr=(UINT16*)gt_modbus_slave_read_write[register_addr-MODBUS_SLAVE_READ_WRITE_START_ADDRESS];
   }
   else if (register_addr>=MODBUS_SLAVE_DDF_START_ADDRESS && register_addr<MODBUS_SLAVE_LAST_ADDRESS)
   {
        data_ptr=(UINT16*)&gt_modbus_slave_ddf_file[register_addr-MODBUS_SLAVE_DDF_START_ADDRESS];
   }
   return data_ptr;
}
//**********************************************************************************************************************
...
void modbus_slave_read_input_registers()
{
    ...
    if (MODBUS_SLAVE_READONLY_REMAP_ADDRESS+MODBUS_SLAVE_READONLY_NUM==MODBUS_SLAVE_READWRITE_REMAP_ADDRESS)
    {
        if (register_addr>=MODBUS_SLAVE_READONLY_REMAP_ADDRESS && register_addr<MODBUS_SLAVE_READWRITE_REMAP_ADDRESS+MODBUS_SLAVE_READ_WRITE_NUM)
        {
          check=1;
          type=4;
        }
    ...
    if (check==1)
    {
        ...
        if (type==3 && register_value+register_addr>MODBUS_SLAVE_READWRITE_REMAP_ADDRESS+MODBUS_SLAVE_READ_WRITE_NUM)
            check2=1;
        if (type==4 && register_value+register_addr>MODBUS_SLAVE_READWRITE_REMAP_ADDRESS+MODBUS_SLAVE_READ_WRITE_NUM)
            check2=1;
...
void modbus_slave_receiving_and_checking()
{
    ...
    {
        if (gb_modbus_slave_buffer[MODBUS_SLAVE_BYTE_NAME_ADDRESS]!=MODBUS_SLAVE_ID)
        {
...
UINT8 modbus_slave_sending()
{
    if (gb_modbus_slave_state==MODBUS_SLAVE_STATE_SENDING)
    {
        ...
        else
        {
            ...
            if (gb_modbus_slave_ptr<gb_modbus_slave_response_length)
            {
                if (gb_modbus_slave_char_ptr==0)
                {
                    char_value=MODBUS_SLAVE_ASCII_START_CHAR;
                }
                else
                {
                    value=gb_modbus_slave_buffer[gb_modbus_slave_ptr];
                    if (((gb_modbus_slave_char_ptr-1)%2)==1)
                    {
                        value=(value&MODBUS_SLAVE_LOW_FOUR_BITS);
                        gb_modbus_slave_ptr++;
                    }
                    else
                    {
                        value=(value&MODBUS_SLAVE_HIGH_FOUR_BITS)>>MODBUS_SLAVE_HALF_BYTE_BITS;
                    }
                    if (value<=MODBUS_SLAVE_HEX_9)
                        char_value=value+MODBUS_SLAVE_ASCII_ZERO_CHAR;
                    if (value>=MODBUS_SLAVE_HEX_A && value<=MODBUS_SLAVE_HEX_F)
                        char_value=value+MODBUS_SLAVE_ASCII_A_CHAR-MODBUS_SLAVE_HEX_TEN;
                }
            }
            else // sending CR & LF
            {
                if (((gb_modbus_slave_char_ptr-1)%2)==1)
                {
                    char_value=MODBUS_SLAVE_LF_CODE;
                    gb_modbus_slave_state=MODBUS_SLAVE_STATE_BACK_TO_STOP;
                    return_value=0;
                }
                else
                {
                    char_value=MODBUS_SLAVE_CR_CODE;
                }
            } // if (gb_modbus_slave_ptr<gb_modbus_slave_response_length)
            gb_modbus_slave_char_ptr++;
            MODBUS_SLAVE_SEND_OUT_API(char_value);
            MODBUS_SLAVE_RESET_TIMEOUT_TIMER_API();
            return return_value;
...
{
    MODBUS_SLAVE_UART_HIGH_SPEED_API();
    MODBUS_SLAVE_SPEED_UP_API();
    MODBUS_SLAVE_RX_INTERRUPT_DISABLE_API();
    MODBUS_SLAVE_RESET_TIMEOUT_TIMER_API();
